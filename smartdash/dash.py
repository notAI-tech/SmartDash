...
def fetch_logs(app_name, last_n_hours):
    logs_data = requests.get(f"http://localhost:6788/logs?app_name={app_name}&last_n_hours={last_n_hours}").json()
...
def fetch_ml_inputs_outputs(app_name, last_n_hours):
    logs_data = requests.get(f"http://localhost:6788/ml_inputs_outputs?app_name={app_name}&last_n_hours={last_n_hours}").json()
    # Convert timestamp to datetime format
    for log in logs_data:
        log['timestamp'] = datetime.fromtimestamp(log['timestamp'])
    return logs_data
...
def fetch_timers(app_name, last_n_hours):
    timers_data = requests.get(f"http://localhost:6788/timers?app_name={app_name}&last_n_hours={last_n_hours}").json()
    return timers_data
...
def calculate_time_per_stage(grouped_timers, finished_no_failed_u_ids, finished_failed_u_ids):
    unique_stages = set()
...
def calculate_stage_times(timers_data):
    ...
    for stage, timestamps in stages.items():
        ...
        median_time = time_diffs[len(time_diffs) // 2] if len(time_diffs) % 2 == 1 else (time_diffs[len(time_diffs) // 2 - 1] + time_diffs[len(time_diffs) // 2]) / 2
        stage_times.append({"stage": stage, "avg_time": avg_time, "median_time": median_time})
    return stage_times
...
def main():
    ...
    app_name = st.sidebar.selectbox("Select App", ["analytics"], index=0)
    time_range = st.sidebar.selectbox("Select Time Range", ["8 hours", "12 hours", "Last day", "Last week"], index=0)
    ...
    time_per_stage = calculate_time_per_stage(grouped_timers, finished_no_failed_u_ids, finished_failed_u_ids)
    ...
    fig = px.pie(names=categories.keys(), values=categories.values(), title="Timers Categorization")
    ...
    for stage in time_per_stage_df.columns:
        if stage != "u_id":
            fig.add_trace(go.Scatter(x=time_per_stage_df["u_id"], y=time_per_stage_df[stage], name=stage))

    # Add a line for the total time
    # fig.add_trace(go.Scatter(x=total_times_df["u_id"], y=total_times_df["total_time"], name="Total Time"))

    fig.update_layout(title="Time Taken per Stage and Total Time", xaxis_title="U_ID", yaxis_title="Time")
    graphs.append(fig)
